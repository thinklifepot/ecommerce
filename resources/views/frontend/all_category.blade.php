                                ...
                                @foreach ($categories as $key => $category)
                                    <li class="@php if($key == 0) echo 'active' @endphp">
                                        <a href="#{{ $key }}" class="row no-gutters align-items-center">
                                            ...
                                            <div class="col-md-9">
                                                <div class="cat-name">{{  translate($category->name) }}</div>
                                            </div>
                                        </a>
                                    </li>
                                @endforeach
            ...
            @foreach ($categories as $key => $category)
                ...
                <div class="mb-3 bg-white">
                    <div class="sub-category-menu active" id="more">
                        <h3 class="category-name border-bottom pb-2"><a href="{{ route('products.category', $category->slug) }}">{{  translate($category->name) }}</a></h3>
                        <div class="row">
                            @foreach ($category->subcategories as $key => $subcategory)
                            <div class="col-lg-4 col-6">
                                <h6 class="mb-3"><a href="{{ route('products.subcategory', $subcategory->slug) }}">{{  translate($subcategory->name) }}</a></h6>
                                <ul class="mb-3">
                                    @foreach ($subcategory->subsubcategories as $key => $subsubcategory)
                                    <li class="w-100"><a href="{{ route('products.subsubcategory', $subsubcategory->slug) }}" >{{  translate($subsubcategory->name) }}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                            @endforeach
