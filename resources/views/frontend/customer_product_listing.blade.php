                                    ...
                                    <ul>
                                        @if(!isset($category_id) && !isset($subcategory_id) && !isset($subsubcategory_id))
                                            @foreach(\App\Category::all() as $category)
                                                <li class=""><a href="{{ route('customer_products.category', $category->slug) }}">{{  translate($category->name) }} ({{ count($category->classified_products) }})</a></li>
                                            @endforeach
                                        @endif
                                        @if(isset($category_id))
                                            ...
                                            @foreach (\App\Category::find($category_id)->subcategories as $key2 => $subcategory)
                                                <li class="child"><a href="{{ route('customer_products.subcategory', $subcategory->slug) }}">{{  translate($subcategory->name) }} ({{ count($subcategory->classified_products) }})</a></li>
                                            @endforeach
                                        @endif
                                        @if(isset($subcategory_id))
                                            <li class="active"><a href="{{ route('customer.products') }}">{{ translate('All Categories')}}</a></li>
                                            ...
                                            @foreach (\App\SubCategory::find($subcategory_id)->subsubcategories as $key3 => $subsubcategory)
                                                <li class="child"><a href="{{ route('customer_products.subsubcategory', $subsubcategory->slug) }}">{{  translate($subsubcategory->name) }} ({{ count($subsubcategory->classified_products) }})</a></li>
                                            @endforeach
                                        @endif
                                        @if(isset($subsubcategory_id))
                                            <li class="active"><a href="{{ route('customer.products') }}">{{ translate('All Categories')}}</a></li>
                                        ...
                                        <div class="product-box-2 bg-white alt-box my-md-2">
                                            <div class="position-relative overflow-hidden">
                                                <a href="{{ route('customer.product', $product->slug) }}" class="d-block product-image h-100 text-center" tabindex="0">
                                                    <img class="img-fit lazyload" src="{{ asset('../frontend/images/placeholder.jpg') }}" data-src="{{ asset($product->thumbnail_img) }}" alt="{{  translate($product->name) }}">
                                                </a>
                                            </div>
                                            <div class="p-md-3 p-2">
                                                ...
                                                <h2 class="product-title p-0">
                                                    <a href="{{ route('customer.product', $product->slug) }}" class=" text-truncate">{{  translate($product->name) }}</a>
                                                </h2>
                                            </div>
                                        </div>
                                    </div>
